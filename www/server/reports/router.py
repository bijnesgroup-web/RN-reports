...
async def upload_pdf_zip(
    ...
    """
    Upload a zip file containing PDF reports named by their report_no (e.g., 00J496065703.pdf).
    Extracts and stores them in ./uploads/pdfs/.
    Also logs uploaded PDFs in database.
    """
    if not file.filename.lower().endswith(".zip"):
    ...
    try:
        with zipfile.ZipFile(buf, "r") as zf:
            ...
            for name in pdf_files:
                ...
                report_no, ext = os.path.splitext(base_name)
...
def upload_xlsx(
    file: UploadFile = File(...),
    company_logo: UploadFile = File(None),   # ⬅ ADD NEW LOGO FILE
    diamond_type: Optional[str] = Form(None),
    comment: str = Form(None),
    isecopy: bool = Form(False),
    notice_image: bool = Form(False),
    igi_logo: bool = Form(False),            # ⬅ NEW form field
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_current_user)
):
    # --------------------------------------------------------
    ...
    if company_logo:
        ...
        # Save file inside uploads/logo
        company_logo_filename = f"company_logo_{datetime.utcnow().timestamp()}.{ext}"
        logo_path = os.path.join(logo_dir, company_logo_filename)

        # company_logo is a SpooledTemporaryFile-like - use .file.read()
        with open(logo_path, "wb") as f:
            f.write(company_logo.file.read())

    # --------------------------------------------------------
    # Existing XLSX upload processing
    # --------------------------------------------------------

    diamond_type_used = diamond_type.strip() if isinstance(diamond_type, str) and diamond_type.strip() else None
    ...
    for idx, row in df.iterrows():
        ...
        elif "comment" in normalized_cols:
            col_name = normalized_cols["comment"]
            comment_text = str(row.get(col_name, "")).strip()
            if comment_text:
                comment_val = comment_text

        num_diamonds = row.get("No Of Diamonds", 0)
        try:
            num_diamonds_int = int(float(str(num_diamonds).strip()))
        except Exception:
            num_diamonds_int = 0
        ...
        base_diamond_phrase = "Natural Diamonds"
        diamonds_phrase = diamond_type_used if diamond_type_used else base_diamond_phrase

        desc = (
            f"One {row.get('Metal Color', '')} {row.get('Jewelry Description', '')}, "
        ...
        tot = f"{row.get('Diamond Weight', '')}"
        color = row.get("Color Criteria", "")
        clarity = row.get("Clarity Criteria", "")
        report_no = gen_report_no()

        # Determine igi_logo per row: prefer explicit form param; else check sheet column
        igi_logo_val = bool(igi_logo)
        if not igi_logo_val:
            for col_key in ("igi_logo", "igi logo", "igi-logo"):
                if col_key in normalized_cols:
                    val = row.get(normalized_cols[col_key])
                    if pd.notna(val):
                        if isinstance(val, bool):
                            igi_logo_val = bool(val)
                        else:
                            igi_logo_val = str(val).strip().lower() in {"true","1","yes","y","t"}
                    break
        ...
        img = None
        try:
            for media_name, img_data in image_map.items():
                if style and style in media_name:
                    img = Image.open(io.BytesIO(img_data))
                    break
                if str(idx + 1) in media_name:
                    img = Image.open(io.BytesIO(img_data))
                    break
        except Exception:
            img = None
        ...
        cell_name = f"B{idx + 2}"
        if img is None and image_loader.image_in(cell_name):
            try:
        ...
        # --------------------------------------------------------
        # 2) ADD company_logo & igi_logo fields when saving report
        # --------------------------------------------------------
        report = Report(
            report_no=report_no,
            description=desc,
            ...
            company_logo=company_logo_filename,   # ⬅ SAVE HERE (if uploaded)
            comment=comment_val,
